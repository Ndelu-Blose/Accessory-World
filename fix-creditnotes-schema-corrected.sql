...
/* Initialize AmountRemaining from RemainingAmount if it exists */
IF COL_LENGTH('dbo.CreditNotes','RemainingAmount') IS NOT NULL
AND COL_LENGTH('dbo.CreditNotes','AmountRemaining') IS NOT NULL
BEGIN
  UPDATE CN
    SET AmountRemaining = CN.RemainingAmount
  FROM dbo.CreditNotes CN
  WHERE CN.AmountRemaining = 0 AND CN.RemainingAmount > 0;
END;
